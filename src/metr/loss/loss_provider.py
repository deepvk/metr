...
class LossProvider:
    ...
    def get_loss_function(
        self, model, colorspace="RGB", reduction="sum", deterministic=False, pretrained=True, image_size=None
    ):
        """
        ...
        if model.lower() in ["l2"]:
            ...
            if is_greyscale:
                loss = GreyscaleWrapper(
                    PNetLin, (), {"pnet_type": "squeeze", "reduction": reduction, "use_dropout": False}
                )
                if pretrained:
