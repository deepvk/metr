...
class WatsonDistanceVgg(nn.Module):
    ...
    def __init__(self, trainable=False, reduction="sum"):
        ...
        self.shift = nn.Parameter(torch.Tensor([-0.030, -0.088, -0.188]).view(1, 3, 1, 1), requires_grad=False)
        self.scale = nn.Parameter(torch.Tensor([0.458, 0.448, 0.450]).view(1, 3, 1, 1), requires_grad=False)
