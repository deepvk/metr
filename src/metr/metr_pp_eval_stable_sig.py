    ...
    if params.eval_bits:
        ...
        if params.attack_mode == "all":
            attacks = {
                ...
                "overlay_text": lambda x: utils_img.overlay_text(
                    x, [76, 111, 114, 101, 109, 32, 73, 112, 115, 117, 109]
                ),
        ...
        if params.with_tracking:
            ...
            log_dict = (
                df_mean.drop(columns=["img"])
                .rename(columns={"bit_acc_none": "Bit_acc", "word_acc_none": "Word_acc"})
                .to_dict("records")[0]
            )
            wandb.log(log_dict)
...
def get_parser():
    ...
    aa(
        "--img_dir_nw",
        type=str,
        default="/checkpoint/pfz/2023_logs/0104_aisign_sd_txt2img/_ldm_decoder_ckpt=0_config=0_ckpt=0/samples",
        help="",
    )
    ...
    aa(
        ...
        type=str,
        default="output/",
        help="Output directory for logs and images, when doing eval images (Default: /output)",
    )
