...
def build_optimizer(name, model_params, **optim_params):
    ...
    torch_optimizers = sorted(
        name
        for name in torch.optim.__dict__
        if name[0].isupper() and not name.startswith("__") and callable(torch.optim.__dict__[name])
    )
...
def adjust_learning_rate(optimizer, step, steps, warmup_steps, blr, min_lr=1e-6):
    ...
    if step < warmup_steps:
        ...
        lr = min_lr + (blr - min_lr) * 0.5 * (1.0 + math.cos(math.pi * (step - warmup_steps) / (steps - warmup_steps)))
...
def get_dataloader(
    data_dir, transform, batch_size=128, num_imgs=None, shuffle=False, num_workers=4, collate_fn=collate_fn
):
