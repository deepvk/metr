...
def get_parser():
    ...
    aa(
        "--msg_decoder_path",
        type=str,
        default="dec_48b_whit.torchscript.pt",
...
def main(params):
    ...
    for ii_key in range(params.num_keys):
        ...
        if not params.checkpoint_name:
            torch.save(
                ldm_decoder.state_dict(), os.path.join(params.output_dir, f"ldm_decoder_checkpoint_{ii_key:03d}.pth")
            )
        else:
